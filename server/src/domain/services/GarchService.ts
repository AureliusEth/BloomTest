...
export class GarchService {
  ...
  calculateVolatility(returns: number[]): Volatility {
    ...
    const lastVariance = this.calculateConditionalVariance(returns, params)[returns.length - 1];
    
    const nextVariance = params.omega + params.alpha * Math.pow(lastReturn, 2) + params.beta * lastVariance;
  ...
  private calculateConditionalVariance(returns: number[], params: { omega: number, alpha: number, beta: number }): number[] {
    ...
    for (let i = 1; i < returns.length; i++) {
      ...
      const nextVar = params.omega + params.alpha * prevReturnSq + params.beta * prevVar;
      variances.push(nextVar);
    }
    return variances;
  }

  private logLikelihood(returns: number[], params: { omega: number, alpha: number, beta: number }): number {
    // Constraint check
    if (params.alpha + params.beta >= 1 || params.omega <= 0 || params.alpha < 0 || params.beta < 0) {
      return -Infinity;
    }
    ...
    for (let i = 0; i < returns.length; i++) {
      ...
      logL += -0.5 * (Math.log(variances[i]) + Math.pow(returns[i], 2) / variances[i]);
    }

    return logL;
  }

  private optimizeParams(returns: number[], initial: { omega: number, alpha: number, beta: number }) {
